...
MyFunctions.WaitForBobber = function()
  ...
  if Bobber then
    ...
    MyVariables.State = "CLICK_BOBBER"
  end
end

MyFunctions.AutoFish = function()
  if MyVariables.InMiniGame then return end
...
MyFunctions.GetBobberPos = function()
  local Bobber = DesktopBtn:FindFirstChild("Bobber"); if not Bobber then 
    MyVariables.FoundBobber = false
  return end

  MyFunctions.DebugPrint("debug", "Bobber found, getting position.")
  MyVariables.FoundBobber = true
    
  return Bobber.AbsolutePosition.X + Bobber.AbsoluteSize.X / 2, Bobber.AbsolutePosition.Y + Bobber.AbsoluteSize.Y / 2
end

MyFunctions.ClickBobber = function()
  local Bobber = DesktopBtn:FindFirstChild("Bobber")
  if not Bobber then
    MyVariables.State = "WAIT_BOBBER"
    return
  end
  ...
  MyFunctions.DebugPrint("debug", "Bobber clicked")

  MyVariables.State = "MINIGAME"
end

MyFunctions.AutoCatchFish = function()
  if MyVariables.State ~= "MINIGAME" then return end

  local overlay = FishOsScreenGui:FindFirstChild("MinigameOverlay")
  if not overlay then
    MyVariables.State = "WAIT_BOBBER"
    return
  end

  local bar = overlay:FindFirstChild("BarOuter")
  local fishingBar = bar and bar:FindFirstChild("FishingBar")
  local catchZone = fishingBar and fishingBar:FindFirstChild("CatchZone")
  local fish = fishingBar and fishingBar:FindFirstChild("FishIcon")

  if not (catchZone and fish) then
    MyVariables.State = "WAIT_BOBBER"
    return
  end

  local catchPos = catchZone.AbsolutePosition.X + catchZone.AbsoluteSize.X / 2
  local fishPos = fish.AbsolutePosition.X + fish.AbsoluteSize.X / 2

  if math.abs(catchPos - fishPos) <= 10 then
    mouse1click()
